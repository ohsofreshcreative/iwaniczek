...
<h2 class="header-line mb-10" >Koszyk</h2>
	...
	<div class="flex-1">
		<form class="woocommerce-cart-form" action="{{ esc_url( wc_get_cart_url() ) }}" method="post">
			...
			<table class="shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
				<thead class="bg-white rounded-t-2xl overflow-hidden">
					<tr>
				...
				<tbody>
					...
					@endphp
					<tr class="woocommerce-cart-form__cart-item bg-white rounded-b-2xl {{ esc_attr(apply_filters('woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key)) }}">
			...
			<div class="cart-actions-wrapper flex flex-col md:flex-row items-center justify-between gap-4 mt-6 p-6 bg-secondary rounded-2xl overflow-hidden shadow-xs border border-gray-800">
				@if ( wc_coupons_enabled() )
				<div class="coupon flex gap-3 w-full md:w-auto">
					<label for="coupon_code" class="screen-reader-text">{{ esc_html__('Coupon:', 'woocommerce') }}</label>
					<input type="text" name="coupon_code" class="input-text !m-0 !px-4 !py-3 border border-secondary text-primary" id="coupon_code" value="" placeholder="{{ esc_attr__('Coupon code', 'woocommerce') }}" />
					<button type="submit" class="button btn btn-secondary {{ esc_attr( wc_wp_theme_get_element_class_name('button') ? ' ' . wc_wp_theme_get_element_class_name('button') : '' ) }}" name="apply_coupon" value="{{ esc_attr__('Apply coupon', 'woocommerce') }}">
						{{ esc_html__('Apply coupon', 'woocommerce') }}
					</button>
					@php do_action('woocommerce_cart_coupon'); @endphp
				</div>
				@endif
				<button type="submit" class="button ml-auto {{ esc_attr( wc_wp_theme_get_element_class_name('button') ? ' ' . wc_wp_theme_get_element_class_name('button') : '' ) }}" name="update_cart" value="{{ esc_attr__('Update cart', 'woocommerce') }}">
