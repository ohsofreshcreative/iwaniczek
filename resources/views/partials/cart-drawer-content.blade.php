...
@else
    ...
    <div class="flex-1 overflow-y-auto divide-y divide-gray-100 px-6 py-4 space-y-4 custom-scrollbar">
        @foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item)
            ...
            @endphp
            <div class="flex gap-4 py-4 first:pt-2 last:pb-2 group relative items-center border-b border-gray-700">
                ...
                <div class="flex-1 min-w-0 pr-6">
                    @if ($permalink)
                        <a href="{{ $permalink }}" class="font-bold text-sm text-primary hover:text-primary-hover transition-colors block truncate">
                            {{ $product_name }}
                        </a>
                    @else
                        <span class="font-bold text-sm text-primary block truncate">{{ $product_name }}</span>
                    @endif
                    <div class="__summary text-base text-white mt-1 flex items-center gap-1.5">
                        <span class="font-header">Ilość: <strong class="text-primary">{{ $cart_item['quantity'] }}</strong></span>
                        <span class="text-gray-500 font-bold">•</span>
                        <span class="!text-white">{!! $product_price !!}</span>
                    </div>
                    <div class="__price mt-1 font-bold !text-xs !text-white mt-1 flex items-center gap-1.5 text-sm text-primary">
                        {!! $product_price_total !!}
                    </div>
                </div>
    ...
    <div class="border-t border-gray-100 p-6 bg-secondary space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <a href="{{ wc_get_cart_url() }}" class="bg-white! border-2 border-gray-200 text-primary! hover:bg-gray-100 hover:border-gray-300 transition-colors text-center font-bold py-3 !px-3 rounded-lg text-sm shadow-xs flex items-center justify-center !w-full">
               Koszyk
            </a>
            <a href="{{ wc_get_checkout_url() }}" class="btn btn-primary text-center font-bold !py-3 !px-3 !rounded-lg text-sm shadow-xs flex items-center justify-center !w-full">
                Zamówienia
            </a>
        </div>
    </div>
